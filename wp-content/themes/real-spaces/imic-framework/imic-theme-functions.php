  ...
  <div id="content" class="content full">
        <div class="container">
            <div class="page">
                <div class="row">
                    <div class="col-md-12">
                    	<h2>'.__('You must have Admin/Agent rights to view this page.','framework').'</h2>
					</div>
                </div>
            </div>
        </div>
    ...
    function property_author_column_content($column_name, $post_id) {
        switch ($column_name) {
			...
			case 'pid':
				...
				if($property_id=='') { $new_id = 1648+$post_id; update_post_meta($post_id,'imic_property_site_id','rs-'.$new_id); }
				echo $property_id;
				break;
        }
    }
...
if(!function_exists('imic_search_filter')){
   ...
   global $imic_options;
        if($query->is_search()) {
            ...
            $propery_type= ($propery_type == __('Type', 'framework')) ? '' :$propery_type;
            $propery_location = isset($_GET['propery_location'])?$_GET['propery_location']:'';
            $propery_location = ($propery_location == __('Location', 'framework')) ? '' : $propery_location;
...
function add_property_id($post_id) {
    ...
    update_post_meta($post_id, 'imic_property_site_id', 'rs-'.$property_id); }
...
echo __('Property ID','framework').': '.get_post_meta($id,'imic_property_site_id',true);
if(!empty($property_address)){
echo '<br/>'.__('Property Address').': '.$property_address;
}
if($property_pin!='') { echo '<br/>'.__('Pin Code ').': '.$property_pin; }
if($property_city!='') { echo '<br/>'.__('Property City ').': '.$property_city; }
}
endif;
/* Add Custom City to property By Id on save
      ================================= */  
if(!function_exists('imicAddCustomCityToPropertyId')):
function imicAddCustomCityToPropertyId($post_id) {
    global $wpdb;
     $custom_city = get_post_meta($post_id,'imic_property_custom_city',true);
       if(!empty($custom_city)){
           wp_set_object_terms($post_id,$custom_city,'city-type'); 
       }
 }
 add_action('save_post', 'imicAddCustomCityToPropertyId');
 endif;
/* 
