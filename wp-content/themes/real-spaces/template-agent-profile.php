...
if (!empty($_POST['action'])) {
	...
	$agentDataKeys = array('mobile-phone', 'work-phone', 'fb-link', 'twt-link', 'gp-link', 'msg-link');
	$agentDataValues = array($agentMobileNo, $agentWorkNo, $agentFBLink, $agentTWTLink, $agentGPLink, $agentMSGLink);
	...
	if(!empty($_FILES)) {
		...
		|| ($_FILES["agent-image"]["type"] == "image/jpg") ) && ($_FILES["agent-image"]["size"] < 256000) && in_array($extension, $allowedExts))
		{
			...
			update_user_meta($user_ID,'agent-image',$user_image);
			}
		}
	}
	$errmsg = '';
                      ...
                      <div class="padding-as25 margin-30 lgray-bg">
                          ...
                          <div class="row">
                              ...
                              <div class="col-md-8 col-sm-8 submit-image">
                                   <?php $userImgSrc='';
									$userImg = esc_attr(get_the_author_meta('agent-image', $user_ID));
                                                                        if(!empty($userImg)) {
										$userLoadedImgSrc = wp_get_attachment_image_src($userImg, '600-400-size');
										$userImgSrc = $userLoadedImgSrc[0];
									}
                                                                        echo '<div class="image-placeholder"><img src="'.$userImgSrc.'" class="image-placeholder" id="agent-image" alt="IMAGE NOT FOUND"/></div>';
									?>
